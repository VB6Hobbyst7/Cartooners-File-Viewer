...
'This class contains the LBM related procedures.
Public Class LBMClass
         ...
         If Not LBMFilePath = Nothing Then
            With LBMFile
               ...
               Using DataStream As New BinaryReader(New MemoryStream(.Data.ToArray()))
                  ...
                  .IFFFileSize = NumberFromBytes(DataStream.ReadBytes(&H4%).ToList())
                  ...
                  If .IFFIdentifier = IFF_IDENTIFIER AndAlso .IFFType = IFF_TYPE Then
                     Do While DataStream.BaseStream.Position < DataStream.BaseStream.Length - &H1%
                        IFFRecord = New IFFRecordStr With {.Offset = CInt(DataStream.BaseStream.Position), .Identifier = DataStream.ReadChars(&H4%), .Size = NumberFromBytes(DataStream.ReadBytes(&H4%).ToList())}
                        ...
                        Select Case IFFRecord.Identifier
                           Case BMHD_IFF_RECORD
                              .ImageO = New Bitmap(NumberFromBytes(DataStream.ReadBytes(&H2%).ToList()), NumberFromBytes(DataStream.ReadBytes(&H2%).ToList()))
                           ...
                           Case TINY_IFF_RECORD
                              .Thumbnail = New Bitmap(NumberFromBytes(DataStream.ReadBytes(&H2%).ToList()), NumberFromBytes(DataStream.ReadBytes(&H2%).ToList()))
   ...
   Private Function Import(ImportPath As String) As String
      Try
         ...
         With Data
            ...
            .AddRange(NumberToBytes(BMHD_IFF_RECORD_SIZE + LBM_4_BIT_CMPA_IFF_RECORD_SIZE + LBMBodyIFFRecordSize + &H18%, &H4%))
            .AddRange(TEXT_TO_BYTES((IFF_TYPE)))

            .AddRange(TEXT_TO_BYTES((BMHD_IFF_RECORD)))
            .AddRange(NumberToBytes(BMHD_IFF_RECORD_SIZE, &H4%))
            .AddRange(NumberToBytes(ImageO.Width, &H2%))
            .AddRange(NumberToBytes(ImageO.Height, &H2%))
