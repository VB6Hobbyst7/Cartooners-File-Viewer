...
Public Class ActorClass
   ...
   Private ReadOnly GET_OFFSET As Func(Of List(Of Byte), LocationsE, Integer) = Function(Data As List(Of Byte), Position As LocationsE) LocationsE.BaseOffset + GetLEDWord(Data, Position)   'This procedure returns the relative offset at the specified position.
      ...
      Try
         ...
         If Refresh Then
            If LocationsE.ActionCount < DataFile().Data.Count AndAlso LocationsE.WayCount < DataFile().Data.Count Then
               ReDim CurrentAnimationRecordLists(0 To 2, GetLEWord(DataFile().Data, LocationsE.ActionCount) - 1, GetLEWord(DataFile().Data, LocationsE.WayCount) - 1)
   ...
   Private Function AnimationRecords(Optional Refresh As Boolean = False) As List(Of AnimationRecordStr)
      Try
         ...
         If Refresh Then
            ...
            For Position As Integer = LocationsE.ImageRecords + GetLEDWord(DataFile().Data, LocationsE.ImageRecordsSize) To GET_OFFSET(DataFile().Data, LocationsE.AnimationRecordListOffset) - ANIMATION_RECORD_LENGTH Step ANIMATION_RECORD_LENGTH
      ...
      Try
         ...
         With New StringBuilder
            ...
            For Record As Integer = 0 To ImageRecords().Count - 1
               ...
               Size = GetLEWord(DataFile().Data, Offset)
      ...
      Try
         ...
         For Each Record As ImageRecordStr In ImageRecords()
            With Record
               ...
               NewText.Append($"{GetLEWord(DataFile().Data, .DataOffset),16}")
   ...
   Private Sub DisplayInformationMenu_Click(sender As Object, e As EventArgs) Handles DisplayInformationMenu.Click
      Try
         Dim ActionCount As Integer? = If(LocationsE.ActionCount < DataFile().Data.Count, GetLEWord(DataFile().Data, LocationsE.ActionCount), Nothing)
         Dim ImageCount As Integer? = If(LocationsE.ImageCount < DataFile().Data.Count, GetLEWord(DataFile().Data, LocationsE.ImageCount), Nothing)
         Dim WayCount As Integer? = If(LocationsE.WayCount < DataFile().Data.Count, GetLEWord(DataFile().Data, LocationsE.WayCount), Nothing)
      ...
      Try
         ...
         For Record As Integer = 0 To ImageRecords().Count - 1
            With ImageRecords()(Record)
               ...
               Draw4BitImage(Decompress(DataFile().Data, .DataOffset + &H2%, GetLEWord(DataFile().Data, .DataOffset)), .Width, .Height, Palette(), .BytesPerRow, TRANSPARENT_INDEX, TransparentColor()).Save(Path.Combine(ExportPath, ImageFiles(ImageFiles.Count - 1)), Imaging.ImageFormat.Png)
            End With
         Next Record
   ...
   Private Function ImageRecords(Optional Refresh As Boolean = False) As List(Of ImageRecordStr)
      Try
         ...
         If Refresh Then
            ...
            For Position As Integer = LocationsE.ImageRecords To LocationsE.ImageRecords + (GetLEDWord(DataFile().Data, LocationsE.ImageRecordsSize) - IMAGE_RECORD_LENGTH) Step IMAGE_RECORD_LENGTH
               CurrentImageRecords.Add(New ImageRecordStr With {.DataOffset = GetLEDWord(DataFile().Data, Position), .Width = GetLEWord(DataFile().Data, Position + &H8%), .Height = GetLEWord(DataFile().Data, Position + &H6%), .BytesPerRow = GetLEWord(DataFile().Data, Position + &H4%)})
            Next Position
         End If
               ...
               Select Case Template(Line).ToLower
                  ...
                  Case "[images]"
                     ...
                     Do
                        ...
                        Compressed = New List(Of Byte)(Compress(GetIndexes(ImageO, GBRPalette, Transparent)))
